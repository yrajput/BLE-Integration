...
void setConnectable(void)
{
  ...
  const char local_name[] = {AD_TYPE_COMPLETE_LOCAL_NAME, 'P', 'r', 'i', 'y', 'a', 'l', 'S'};
...
void aci_loop() {
  ...
  if (ble_rx_buffer_len) {  //Check if data is available
    ...
    input_length = ble_rx_buffer_len;
    for(int i = 0; i < ble_rx_buffer_len; i++) {
      ble_input[i] = ble_rx_buffer[i];
    }
...
int get_input(char input[]) {
  int len = (input_length > 0) ? input_length : -1;
  if(input_length > 0) {
    for( int i = 0; i < input_length; i++ ) {
      input[i] = ble_input[i];
    }
    
    int diff = strlen(input) - input_length;
    for( int i = 0; i < diff; i++ ) {
      input[i+input_length] = '\0';
    }
